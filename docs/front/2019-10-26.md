...
```
```applyMiddleware```函数接收多个middlewares参数，返回一个store对象。通过createStore创建store对象，middlewareAPI对象将getState和dispatch封装，map遍历中间件时，将middlewareAPI作为参数传入。遍历结束以后，拿到了一个包含所有中间件新返回函数的一个数组，将其赋值给变量chain。我们来看看中间件的核心。

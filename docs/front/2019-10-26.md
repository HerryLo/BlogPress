...
**最好把上面看懂之后，再看中间件部分！！**其实就是对中间件进行分析，createStore函数传参，参数preloadedState是一个函数，而没有参数enhancer的话，preloadedState会被赋值给enhancer
...
```
以```redux-thunk```中间件作为例子，下面就是thunkMiddleware函数的代码：
```javascript

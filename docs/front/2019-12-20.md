...
```
```[hoistStatics](https://github.com/mridgway/hoist-non-react-statics)```函数的作用就是类似于Object.assign，可以这样理解```hoistStatics(targetComponent, sourceComponent)```。上面代码中```ConnectFunction```是核心函数，虽然中间部分代码省略，不过留下的都是精华。
...
以上就是Provice组件和connect组件初次调用时，所经过的实际代码，当然在其中有一些删减，不过基本都有说到。但是，当dispatch被触发时呢，会发生什么？上面部分完全没有说到，下面我们就来看看当```dispatch(action)```调用后，react-redux内部是如何做到store和组件中state的同步变化。
...
dispatch(action)
```
在redux-react中，订阅更新函数[Subscription](https://github.com/reduxjs/react-redux/blob/master/src/utils/Subscription.js#L52)是其中的核心支持函数，它有效的保证connect组件的更新渲染。**下面内容不是精读代码，只是聊一下基本的流程**。
...
const subscription = new Subscription(store)
```
```store```作为参数传入到Subscription构造函数内部，作用就是Subscription内部使用。

### 订阅
在connect组件被调用时，react-redux中的subscription对象，会将```checkForUpdates```更新函数作为参数，传入保存在订阅数组中；同时，redux也会发生相同的操作【在react-redux中也有使用到redux中方法】。下面代码是极度简化版
```javascript
...
```
```notifyNestedSubs```函数是作用是触发react-redux中的通知函数，被作为入参传入到store中，作为订阅函数保存在
nextListeners数组中。
...
```dispatch(action)```发起触发操作后，当然是触发store中dispatch函数了，dispatch中有会遍历redux的nextListeners数组，触发其中通知函数调用；同时，这又会导致react-redux中的next数组，被触发遍历调用。两个库基本都是下面的代码
```javascript
...
```
细节可能有所省略，就是先订阅，然后当dispatch函数被调用是，触发订阅函数，完成connect组件的更新和state的 同步变化。

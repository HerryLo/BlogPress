...
在```pedding```状态下，```handleResolved```函数不会执行，只会存储then回调deferred对象等操作，下面我们会讲到。**其中```handle```函数是核心函数, 接收```this```和```new Handler```对象。若在```pedding```状态下，负责注册回调函数，更新回调函数状态**。否则，执行```handleResolved```函数。
...
```
finale函数表示进入结束流程，负责执行then方法的回调函数。同时在上面已经说到，在非```pedding```状态下，执行```handle```函数，实际会是执行```handleResolved```函数。

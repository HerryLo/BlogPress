...
[本文原文地址](https://herrylo.github.io/front/2022-11-17.html);

一直有构建个人博客小程序的想法，但又不想自己开发后端接口，想做到无后端，人比较懒，就没有动手开干。

由于公司原因使用了语雀文档，觉得编辑界面蛮不错，功能强大，`思维导图`、`时序图`、`pdf`都支持，顺便将个人文章上传进行备份。后面在持续使用中，发现提供`文档API`接口，于是萌生想法，是否可以考虑用`语雀接口`构建小程序。在`Postman` 测试了一波，发现字段基本都有，那就开始动手干了。

## uniapp&语雀API

小程序构建选择的时[uniapp](https://uniapp.dcloud.net.cn/)，主要是想尝试一下不同的框架，之前有尝试使用 Taro 构建过下小程序：[垃圾分类小程序](https://github.com/HerryLo/wxSapp)，就想试试其他框架了。之前听过群友聊过[uniapp](https://uniapp.dcloud.net.cn/)，就决定试试。
...
。。。。这就有点拉了，不过看在文档好用的份上，还是**开了99的专业会员**，不然[语雀 API](https://www.yuque.com/yuque/developer/api)接口没办法调用，整体来说还划算吧？吧？吧？

## 小程序界面

界面简单整洁，格调清新。好了不吹了,有兴趣的，底部附有项目源码地址。（主要是不知道怎么设计）
